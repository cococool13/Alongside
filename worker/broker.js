...
export function toolData(result, name) {
  ...
  if (result.structuredContent?.accounts) return result.structuredContent;
  const text = result.content?.find(item => item.type === 'text')?.text;
  const parsed = text && JSON.parse(text);
  if (parsed?.data) return parsed.data;
  if (parsed?.accounts) return parsed;
...
export async function callRobinhood(accessToken, name, args = {}) {
  ...
  try {
    ...
    return toolData(result, name);
  } finally {
    await client.close().catch(() => {});
  }
}

export async function agenticAccount(accessToken, call = callRobinhood) {
  const data = await call(accessToken, 'get_accounts');
